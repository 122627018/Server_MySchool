...
//		Log.i("wang", "进入sendget方法中");
		System.out.println("sendGet的url为："+sendData.getUrl());
...
//							+ entry.getValue());
					System.out.println("sendGet中header--" + entry.getKey() + ":"
							+ entry.getValue());
				}
			}
		...
		try {
			...
			if (sendData.getHeaders() != null
				...
				for (Map.Entry<String, String> entry : sendData.getHeaders()
						.entrySet()) {
					conn.setRequestProperty(entry.getKey(), entry.getValue());
//					System.out.println("header中包含："+entry.getKey()+":"+entry.getValue());
				}
			}
...
//				System.out.println("sendData.getPars() != null");
				out = new PrintWriter(conn.getOutputStream());
				// 发送请求参数
