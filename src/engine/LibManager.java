...
public class LibManager {
	...
	public static Map<String, Object> processException(LibException e,
			String cookie) {
		switch (e.getState()) {
		case 1:// 登录过期
			// System.out.println("登录会话过期");
			return getLibCodePic(true);
	...
	public static <T> Map<String, Object> getResponseMap(int state,
			String error, String cookie, byte[] codeByte, T infos) {
	 ...
	 * @param pars
	 */
	public static Map<String, Object> getBorrowState(String cookie) {
		Map<String, Behave> libPars = getLibPars();
		Behave getBorrowBehave = libPars.get("getborrowstate");
		String url = getBorrowBehave.getUrl();
		...
		try {
			HtmlUtil.FZBorrowStatePage(item, new String(data.getContent(),
					"utf-8"));
			return getResponseMap(200, "", data.getHeaders().get("Cookie"),
					null, item);
...
//	}
	...
	/**
	 * 移动端获取获取验证码
	 * @param isOutLogin  如果是登录过期的那种获取验证码，返回的state不一样
	 * @return 需要返回pic的流和cookie
	 */
	public static Map<String, Object> getLibCodePic(boolean isOutLogin) {
		NetSendData send = new NetSendData();
		...
		try {
			...
			String cookie = data.getHeaders().get("Cookie");
			...
			if (isOutLogin) {
				return getResponseMap(1, "登录会话过期", cookie, picCode, null);
			} else {
				return getResponseMap(200, "", cookie, picCode, null);
			}
		} catch (IllegalStateException e) {
			e.printStackTrace();
		} catch (LibException e) {
			return getResponseMap(e.getState(), e.getMessage(), "", null,
					null);
		}
		return null;
	}
...
//	}
	...
	public static Map<String, Object> Login(String username, String password,
		...
		LoginFormat login = new LoginFormat();

		// 3.从数据库取得所需要的全部参数集合
//		login.setBehaves(getLibPars());
...
//		Behave behave = login.getBehaves().get("login");
		Map<String, Behave> libBehave = getLibPars();
		Behave loginBehave = libBehave.get("login");
		String loginPars = loginBehave.getPars();
		String loginUrl = loginBehave.getUrl();
		...
		loginToSendData.setHost(libBehave.get("host").getUrl());
		...
		try {
			HtmlUtil.parseLibLoginHtml(login, new String(
					receiverData.getContent(), "utf-8"));
			...
			return getResponseMap(e.getState(), e.getMessage(), cookie, null,
					null);
		}
...
//		//
//		// // 6.发送请求，取得服务器返回的数据对应的实体
//		// NetReceiverData receiverData =
//		// MyHttpWebUtil.sendPost(loginToSendData);
//		// // 7.要通过receiverData来判断是否登录成功
//		// try {
...
//	}
	...
	public static Map<String, Behave> getLibPars() {
